...
def start_life_assistant():
    ...
    if platform.system() == "Windows":
        ...
        if not args.frontend_only:
            backend_command = "start cmd /k python backend_loop.py"
            if args.debug:
        ...
        if not args.backend_only:
            frontend_command = "start cmd /k python frontend_assistant.py"
            if args.debug:
        ...
        if not args.frontend_only:
            backend_cmd = "python3 backend_loop.py"
            if args.debug:
                backend_cmd += " --debug"
        ...
        if not args.backend_only:
            frontend_command = "python3 frontend_assistant.py"
            if args.debug:
                frontend_command += " --debug"
        ...
        if not args.frontend_only:
            ...
            if os.system("which gnome-terminal > /dev/null 2>&1") == 0:
                backend_command = f"gnome-terminal -- bash -c 'cd {os.getcwd()} && python3 backend_loop.py"
                if args.debug:
                    backend_command += " --debug"
                ...
                backend_command = f"xterm -T 'Life Assistant Backend' -e 'cd {os.getcwd()} && python3 backend_loop.py"
                if args.debug:
                    backend_command += " --debug"
                backend_command += "; exec bash'"
                ...
                print(f"  python3 backend_loop.py {'--debug' if args.debug else ''}")
