...
DEFAULT_TASKS_FILE = os.path.join(BASE_DIR, 'data-backend', 'tasks.md')
...
class TaskExecutionModel:
    ...
    def execute_directives(self, directives, plan, tasks=None, cron=None, system_memory=None):
        ...
        try:            # Split into thoughts and JSON actions
            thoughts = ""
            actions = []
            action_json = ""
            ...
            if json_start is not None and json_end is not None:
                ...
                try:
                    ...
                    print(f"DEBUG: Successfully parsed JSON actions from code block")
                except json.JSONDecodeError as e:
                    thoughts += f"\nError parsing actions JSON from code block: {e}"
                    print(f"DEBUG: JSON parse error from code block: {e}")
                    actions = []
            else:
                # If no JSON found in code blocks, try advanced extraction techniques
                import re
                
                # Look for arrays with objects that have type and args properties
                json_match = re.search(r'\[\s*\{\s*"type"\s*:', response, re.DOTALL)
                if json_match:
                    ...
                    if json_end > json_start:
                        ...
                        thoughts = response[:json_start].strip()
                        try:
                            actions = json.loads(action_json)
                            print(f"DEBUG: Successfully parsed JSON actions using regex")
                        except json.JSONDecodeError as e:
                            thoughts += f"\nError parsing actions JSON using regex: {e}"
                            print(f"DEBUG: JSON parse error using regex: {e}")
                            actions = []
                            
                            # Try to fix common JSON issues
                            try:
                                # Replace single quotes with double quotes
                                fixed_json = action_json.replace("'", '"')
                                actions = json.loads(fixed_json)
                                print("DEBUG: Fixed JSON by replacing quotes")
                            except:
                                pass
                    else:
                        thoughts = response
                    ...
                    # Last attempt: Try to find any array in the response
                    json_match = re.search(r'\[.*\]', response, re.DOTALL)
                    if json_match:
                        try:
                            actions = json.loads(json_match.group(0))
                            thoughts = response.replace(json_match.group(0), "").strip()
                            print("DEBUG: Found JSON array using simple regex")
                        except Exception as e:
                            thoughts = response
                            print(f"DEBUG: Final JSON extraction failed: {e}")
                            actions = []
                    else:
                        thoughts = response
                        actions = []
